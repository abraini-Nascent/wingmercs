...
export function TextBlock(name: string, text: string, fit: boolean = false, style: TextStyle = Styles.GoldLabel, horrizontalAlignment: number = GUI.TextBlock.HORIZONTAL_ALIGNMENT_LEFT): GUI.TextBlock {
...
export function createSegmentedControl(options: string[], callback: (selectedIndex: number) => void): [FlexContainer, (segment: number) => void] {
