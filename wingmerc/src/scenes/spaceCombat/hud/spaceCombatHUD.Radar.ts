...
import { Entity, EntityForId, EntityUUID, queries, world } from "../../../world/world";
...
export class RadarDisplay {
  ...
  update(playerEntity: Entity, hitPlayer: Set<EntityUUID>, dt: number) {
    ...
    const { direction, up, position, rotationQuaternion, targeting } = playerEntity
    ...
    for (const target of queries.targets) {
      ...
      let targetType: "enemy" | "missile" | "dead" | "player" | "nav" = target.isTargetable
      if (target.deathRattle) {
        targetType = "dead"
      }
      if (targetType == "enemy" && target.teamId == playerEntity.teamId) {
        targetType = "player"
      }
      if (targetType == "nav") {
        continue
    ...
    if (targeting?.destination != undefined && targeting.destination != "") {
      let destination = EntityForId(targeting.destination)
      if (destination != undefined) {
        const radarPosition = mapToRadar(Vector3FromObj(destination.position), Vector3FromObj(position), Vector3FromObj(direction), Vector3FromObj(up), QuaternionFromObj(rotationQuaternion))
        this.drawPointOnDynamicTexture("nav", 100, radarPosition, this.radarTexture, true, false)
      }
    }
    if (frontHit) {
  ...
  drawPointOnDynamicTexture(targetType: string, distance: number, point: {x: number, y: number}, dynamicTexture: DynamicTexture, locked: boolean, update: boolean = false): void {
    ...
    switch (targetType) {
      ...
      case "player":
        ...
        break;
      case "nav":
        colorClose.set(255, 255, 255)
        colorFar.set(255, 255, 255)
        break;
      default:
        colorClose.set(255, 0, 0);
