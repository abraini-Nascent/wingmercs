...
export class SkyboxNebulaSystem implements IDisposable {
  ...
  constructor(scene: Scene) {
    ...
    this.sphere = new VoronoiSphereNebula(scene, 1000, fractalDensity)
  }
  dispose(): void {
    this.sphere.dispose()
  }
...
export function generateWorleyDensityMap(width: number, height: number, featureSize: number): (x: number, y: number) => number {
  const points = [];
