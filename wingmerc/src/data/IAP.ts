...
export function translateIPA(sentence: string, log: boolean = false): string {
  ...
  log && debugLog("[translateIPA] translating:", sentence)
  let words = sentence.split(" ")
  for (let word of words) {
    log && debugLog("[translateIPA] translating word:", word)
    let translatedWord = ""
    let stress = false
    for (let i = 0; i < word.length; ) {
      ...
      log && debugLog(`[translateIPA] match \\${match.glyph}\\ "${match.example}" found for \\${search}\\`)
  ...
  log && debugLog(`[translateIPA] translation: "${translated.join(" ")}"`)
